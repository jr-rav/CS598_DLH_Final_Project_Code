...
if __name__ == '__main__':
  ...
  dtype = {'ITEMID': 'int32',
           ...
           'UNITNAME': 'str',
           'LINKSTO': 'str'}
  defs = pd.read_csv(hp.mimic_dir + 'D_ITEMS.csv', usecols=dtype.keys(), dtype=dtype, nrows=20000)
  print('URINE_OUTPUT')
  ...
  df = pd.read_csv(hp.mimic_dir + 'OUTPUTEVENTS.csv', usecols=dtype.keys(), dtype=dtype, parse_dates=parse_dates, nrows=20000)
  ...
  icustays = pd.read_csv(hp.mimic_dir + 'ICUSTAYS.csv', usecols=dtype.keys(), dtype=dtype, parse_dates=parse_dates, nrows=20000)
